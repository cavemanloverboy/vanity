/* fe.cl — OpenCL port of kernels/ed25519/fe.cu (ref10 25.5-bit field
   ...
   include guards; the only hand-edit is fe_copy_c, a __constant-source
   variant of fe_copy needed because OpenCL 1.2 has no generic address
...
void fe_copy_c(fe h, __constant const int32_t *f);
...
/*
    fe_copy with the source living in __constant memory. Needed because
    OpenCL 1.2 has no generic address space, so a single fe_copy cannot
    accept both private and __constant pointers. Used by ge_select to load
    entries of the precomputed base table.
*/
