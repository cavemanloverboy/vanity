/* fe.cl — OpenCL port of kernels/ed25519/fe.cu (ref10 25.5-bit field
   arithmetic). Generated by stripping the CUDA `__device__` qualifier and
   include guards; the only hand-edit is fe_cmov_c, a __constant-source
   variant of fe_cmov needed because OpenCL 1.2 has no generic address
   space (ge_select reads the precomputed base table from __constant). */

/* Forward declarations. OpenCL C (unlike the CUDA build with fe.h) requires
   these before fe_invert / fe_isnegative / fe_pow22523 reference functions
   defined later in this file. */
void fe_0(fe h);
void fe_1(fe h);
void fe_add(fe h, const fe f, const fe g);
void fe_cmov(fe f, const fe g, unsigned int b);
void fe_cmov_c(fe f, __constant const int32_t *g, unsigned int b);
...
/*
    fe_cmov with the source operand g living in __constant memory. Needed
    because OpenCL 1.2 has no generic address space, so a single fe_cmov
    cannot accept both private and __constant pointers. Used by ge_select
    to conditionally move entries of the precomputed base table.
*/
