...
extern __constant__ uint8_t d_suffix[VANITY_MAX_PATTERNS][VANITY_MAX_PATTERN_LEN];

#ifdef __cplusplus
extern "C"
#endif
void vanity_upload_pattern_table(const uint8_t *blob, uint64_t blob_len);
...
/* Multi-pattern: encode once, OR each pair from __constant__ SoA.
   Forceinlined only into the MULTI=true kernel (the 1-pattern kernel
   never calls this), so its registers cannot leak into that occupancy. */
static __device__ __forceinline__ bool fd_base58_check_match_any_32_words(const uint state[8])
{
    ulong in_leading_0s = 0UL;
#pragma unroll
    for (int i = 0; i < 8; ++i)
    ...
    for (uint32_t pi = 0; pi < n_pat; pi++)
    {
        ...
        for (ulong i = 0UL; i < (ulong)plen; i++)
        {
            ...
            if (d_match_lut[raw_base58[rb_idx]] != d_prefix[pi][i])
            {
                ok = false;
                break;
            }
        ...
        if (ok && slen > 0)
        {
            ...
            {
                if (d_match_lut[raw_base58[tail_start + i]] != d_suffix[pi][i])
                {
                    ok = false;
                    break;
                }
